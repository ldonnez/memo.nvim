...
## Installation with example configuration
...
- Decryption: Before the buffer is displayed, it triggers a BufReadPre event to decrypt the content using GPG.
- Security: The decrypted content exists only in your Neovim buffer.
- Encryption: Upon saving (BufWritePost), the plugin automatically re-encrypts the content back to the disk, ensuring your data is never stored as plain text.
...
You can register a capture command with custom behavior, such as dynamic headers (e.g., timestamps) and configure how the window will split.
